...
const int lidSwitch = 4; // define wakeup pin, in our case pin 4 which is pulled up to VCC with a 10K resistor on the board
const int doorSwitch = 17; // lid switch connected to SDA pin
reasons wakeupReason = undefined; // set to 0 means startup, set to 1 means wakeup from TPL5010, set to 2 means wakeup from user pin, set to 3 means unknown wakeup reason
...
const float AREF = 1.1041;         // internal reference votlage, for better accuracy use the printed value on the sticker!
const float RGND = 20.020;          // voltage divider resistor from A0 to GND, for better accuracy use the printed value on the sticker! Value in K ohms
const float RSUP = 99.90;         // voltage divider resistor from A0 to VCC or UREG depending on solder jumper, for better accuracy use the printed value on the sticker! Value in K ohms
volatile int timerCounts = 0;
bool joined = false;
unsigned long joinEntry;

volatile boxStatus mailboxStatus = empty;
...
void do_send(osjob_t* j) {
  ...
  if (LMIC.opmode & OP_TXRXPEND) {
    ...
    DEBUGPRINT("Reason ");
    DEBUGPRINT(wakeupReason);

    DEBUGPRINT(" mailAvalable ");
    DEBUGPRINT(mailboxStatus);
    ...
    //Prepare uplink data
    byte mydata[3];
    mydata[0] = wakeupReason | mailboxStatus << 7;
    mydata[1] = voltage >> 8;
    mydata[2] = voltage;
    ...
    */
    ...
    // Set wakeupReason value to unknown, (the device woke up because of an unknown reason)
...
void wakeupFromTimer(void) {
  if (digitalRead(TPLWakePin) == HIGH) {
    feedWatchdog();
    wakeupReason = timer;
    timerCounts++;
  }
...
void wakeupFromLidSwitch(void) {
  delay(10);
  if (digitalRead(lidSwitch) == LOW) {
    wakeupReason = lid;
    DEBUGPRINTLN("Lid defined");
  } else wakeupReason = undefined;
}
...
void setup() {
  ...
  while (joined == false) {
    ...
    do_send(&sendjob);
    while (millis() - joinEntry < 20000)  os_runloop_once(); // wait for join
  }
  feedWatchdog();
  DEBUGPRINTLN("Attach Interrupts");
    ...
    if (LMIC.pendMacLen > 0) {
      ...
      switch (wakeupReason) {
        ...
        case timer:
          ...
          if (timerCounts >= TIMERINTERVALS) {
            Serial.println("Timer transmitted");
            do_send(&sendjob);
            timerCounts = 0;
            break;
          case lid:
...
void sleep_long() {
  ...
  DEBUGPRINTLN("Sleep long");
  while ((wakeupReason != door) && (wakeupReason != lid) && (wakeupReason != timer)) {
    //   LowPower.powerDown(SLEEP_FOREVER, ADC_OFF, BOD_OFF);
  }
#else
  LowPower.powerDown(SLEEP_FOREVER, ADC_OFF, BOD_OFF);
#endif
  DEBUGPRINTLN("Sleep long finished");
}
